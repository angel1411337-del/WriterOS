...
""",
    ...
    async def run_with_lcel(
        ...
        """
        LCEL-powered version demonstrating composable chains with RAG preprocessing.

        This method showcases the new LangChain capabilities:
        - RAG retrieval as a preprocessor
        - LCEL chain composition
        - Automatic Pydantic parsing

        Args:
            query: User query about timeline
            vault_id: Vault to search
            retriever: Optional RAG retriever for context

        Returns:
            TimelineExtraction with events and continuity notes

        Example:
            >>> from writeros.services.rag_retriever import RAGRetriever
            >>> retriever = RAGRetriever()
            >>> result = await chronologist.run_with_lcel(
            ...     "What happens in Chapter 5?",
            ...     vault_id=vault_id,
            ...     retriever=retriever
            ... )
            >>> print(result.events[0].title)
        """
        logger.info("chronologist_lcel_run", query=query[:50], vault_id=str(vault_id))

        # Define RAG preprocessor (always use to inject context key)
        async def retrieve_context(inputs: dict) -> dict:
            ...
            if retriever:
                ...
                inputs["context"] = "No additional context available."
...
Extract the timeline of events from the context above.
""",
            ),
        ])

        # Build LCEL chain: retrieve | prompt | llm | parse
        # Always use preprocessor to ensure 'context' variable is populated
        ...
        logger.info(
            "chronologist_lcel_complete",
            events_extracted=len(result.events),
            has_continuity_notes=bool(result.continuity_notes)
        )
